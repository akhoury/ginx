...
function streamData(data, ctx, params, rowCallback) {
    ...
    b = str.indexOf('\n');
    while (b > -1) {
        ctx.hardParseLine(str.substring(a, b), rowCallback, {'file':params.file});
        a = b + 1;
        b = str.indexOf('\n', a);
    }
    params.cursor += a;
...
function streamEnd(overflow, ctx, params, rowCallback, fileCallback, dirCallback) {
    ...
    if (overflow.length) {
        ...
        ctx.hardParseLine(overflow.toString(), rowCallback, {'file':params.file});
    }
    ctx.inProcess--;
    ctx.files--;
    if (ctx.pool.length > 0 && ctx.inProcess < ctx.inProcessMax) {
...
function GinxParser(format, options) {
 ...
 */
GinxParser.prototype.parseFile = function (file, rowCallback, fileCallback, dirCallback, options) {
    var that = this,
        cursor = 0,
        persistTimer = 0,
...
GinxParser.prototype.parseLine = function (line, callback, options) {
 ...
 */
GinxParser.prototype.parseLine_old = function (line, callback, options) {
    var that = this,
