...
GinxParser.prototype.parseFile = function (file, rowCallback, fileCallback, dirCallback, isPool) {
    var that = this,
        cursor, meta,
        strgKey = path.normalize(that.getStorageKeyfromPath(file)), fileStartTime = Date.now();
            ...
            if (size - 1 > cursor || size === -1) {
                ...
                stream.on('data', function (data) {
                    ...
                    for (; i < buffer.length; i++) {
                        ...
                        if (buffer[i] === that.NL) {
                            ...
                            that.parseLine(newbuf.toString(), rowCallback, meta);
                        }
                    }
                ...
                stream.on('end', function () {
                    if (overflow.length) {
                        ...
                        that.parseLine(overflow.toString(), rowCallback, meta);
                    }
                    ...
                    console.log("[GINXPARSER-DEBUG] (3) InProcess: " + that.inProcess + " Pool: " + that.pool.length + " Files: " + that.files);                    
                    if (typeof fileCallback === 'function') {
                        console.log("[GINXPARSER-DEBUG] file "+ file +" ended parsing in: " + (Date.now()-fileStartTime) + " ms");
                        fileCallback(file);
                    } 
                    if(that.files === 0 && typeof dirCallback === 'function'){                      
                        dirCallback();
                    }
                });
