...
GinxParser.prototype.parseLine = function (line, callback, result) {
    ...
    that.delimeters.forEach(function (delim) {
        ...
        if (needle === -1) {
            err = new Error("[GINXPARSER-ERROR] Inconsitent values/delimeters with format: invalid Line: '" + line + "' Somewhere around byte: " + result['__lastCharAt'] + " in File: " + result['__file'] + " Delimeters: " + that.delimeters);
