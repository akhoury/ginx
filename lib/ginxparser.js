 ...
 * runs 5x faster. 
 * Instead of looping through the delimeters, it dynamically creates 'if' statements for each one, this way, the delimeters code will be defined in memory
 *
 * I tested first with a 100 files of 3MB each (the 'large' in ./test/logs)
 * initially took it 35 seconds, now I wait 6.8 seconds only with this new function (3.5s if you don't set fieldsToObjects to true in GinxParser constructor)  
 * as long as I don't console.log each row on the screen and don't consider the time the test needs the setup needs
 ...
 * tested on a Macbook pro 2012, i7, 8gb, 256ssd (oh and parsed a single 1.8GB log file in 15 seconds)
 *
 *
 * May break if Nginx log uses single-quote (') instead of double-quote ("), nginx doesn't by default
 * //TODO - handle both ' and "
 */
function generateParseLine(ctx, delim, attrs, args) {
    var ln = delim.length,
    ...
    funcode += '\nelse {\n\tvar err = new Error("[GINXPARSER-ERROR] Inconsitent values/delimeters with format: invalid Line: " + line + " in File: " + result.__file);';
    funcode += '\n\tif(typeof callback === "function") {\n\t\tcallback(err, result);\n\t} else {\n\t\tresult.__error = err;\n\t\treturn result;\n\t}\n}';
...
function GinxParser(format, params) {
    ...
    this.hardParseLine = generateParseLine(this, this.delimeters, this.attrs, 'line, callback, file');
}
