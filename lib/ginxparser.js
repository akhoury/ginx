...
var fs = require('fs'), path = require('path'), util = require('util'), MEM_STORAGE = {};
...
GinxParser.prototype.parseDir = function (dir, rowCallback, fileCallback){
  var that = this; dir = path.normalize(dir);
    ...
    fs.stat(file, function(err, stats){
      ...
      if (stats.isFile()){
        var stream = fs.createReadStream(file, {'start':cursor}),
            overflow = new Buffer(0), size = getFileSize(stats);
            ...
            for (;i < buffer.length; i++) {
              // slice when you see a newline(nl)
                if (buffer[i] === that.NL) {
                  ...
                  meta = {'__file': file, '__lastCharAt': cursor, '__originalText': newbuf.toString()};
        ...
        stream.on('end', function () {
          if (overflow.length) {
            ...
            meta = {'__file': file, '__lastCharAt': cursor, '__originalText': overflow.toString()};
...
GinxParser.prototype.parseLine = function (line, callback, result){
  var that = this, err, needle, attr, value, i = 0;
  line = line.toString(); 
 ...
 */
 ...
 function convertValue(attr, value){
   ...
   switch (attr) {
     ...
     case 'body_bytes_sent': value = parseInt(value, 10); break;
     case 'status': value = parseInt(value, 10); break;
     default:   
   }
   return value;
 }
...
function getFileSize(stats){
  var str = util.inspect(stats), needle = ",\n  size: ", k, j;
